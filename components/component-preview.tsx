...
async function loadComponent(name: string): Promise<React.ComponentType | null> {
  try {
...
export async function ComponentPreview({
    ...
    return (
      <div className="relative aspect-[4/2.5] w-full overflow-hidden rounded-md border md:-mx-1">
        ...
        <div className="bg-background absolute inset-0 hidden w-[1600px] md:block">
          <iframe src={`/view/new-york/${name}`} className="size-full" />
        </div>
      </div>
    )
  }
  ...
  if (name === "sidekick") {
    const { SidekickProvider, SidekickInset, SidekickTrigger } = await import("@/registry/new-york/blocks/sidekick")
    const { SparklesIcon } = await import("lucide-react")
    
    return (
      <div className={cn("group relative mt-4 mb-12 flex flex-col gap-2 rounded-lg border overflow-hidden", className)} {...props}>
        <div data-slot="preview" className="h-[600px]">
  ...
  return (
    <ComponentPreviewTabs
      ...
      hideCode={hideCode}
      component={
        <Suspense fallback={<div className="animate-pulse bg-muted h-32 w-full rounded" />}>
          <Component />
        </Suspense>
      }
      source={
